...
import { setJobStatus, applyProvisionedPlan, addAudit, addWebhookEvent, beat } from "./db.mjs";
...
export async function processProvisioningJob(job) {
  ...
  // Provisioning now emits an automatic billing webhook so downstream systems are
  // notified the moment the upgrade completes — visible in the timeline + webhook panel.
  await addWebhookEvent({
    eventType: "subscription.activated",
    customerId: SEED.customerId,
    payload: { plan: toPlan, priority: Boolean(priority) },
    signatureValid: true,
    source: "system",
  });
  await addAudit("webhook", `Emitted subscription.activated webhook for the ${toPlan} plan`);
